...
def test2(request):
  ...
  st = []
  [eup(x) for x in tmp if fup(x) == '750' and gup(x) == 1]
  total_750 = sum(int(i) for i in st)
  
  st=[]
  [eup(x) for x in tmp if fup(x) == '748' and gup(x) == 1]
  total_748 = sum(int(i) for i in st)
  st=[]
  [eup(x) for x in tmp if fup(x) == '749' and gup(x) == 1]
  total_749 = sum(int(i) for i in st)
  st=[]
  [eup(x) for x in tmp if fup(x) == '677' and gup(x) == 1]
  total_677 = sum(int(i) for i in st)
  nt=[]
  [mup(x) for x in tmp if fup(x) == '748' and gup(x) == 1]
  mx = t-int(max(nt))
  try:
		m, s = divmod(mx,60)
		h, m = divmod(m, 60)
		diff_time="%d:%02d:%02d" % (h,m,s)
  except:
		diff_time= "0"
		
  return render(request,"test2.html",{'tmp':nt,'max':diff_time,'Count':total_748})
