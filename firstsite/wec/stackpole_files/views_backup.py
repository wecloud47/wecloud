...
#	  pvar = pcount - 1
#	  sqlD = "Select(time) FROM tkb_prodtrak where machine = '%s' AND pcount = '%d' AND qty='%d'" %(machine_list[i], pvar, var) 
#	  cursor.execute(sqlD)
#	  tmp = cursor.fetchall()
#	  tmp2 = tmp[0]
#	  last_2 = tmp2[0]
...
#	  pvar = pcount 
#	  sqlE = "Select(time) FROM tkb_prodtrak where machine = '%s' AND pcount = '%d' AND qty='%d' " %(machine_list[i], pvar, var) 
#	  cursor.execute(sqlE)
#	  tmp = cursor.fetchall()
#	  tmp2 = tmp[0]
#	  last_1 = tmp2[0]	  
	  
#	  cycle[i] = last_1 - last_2
