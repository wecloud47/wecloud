...
def send_email (message_subject, message_text, user, db, toaddrs):
	...
	server.starttls()
	server.ehlo()
	server.login('wecloud.ca@gmail.com', 'Tounces0909')
...
def password_check(request,user1,company1,password1):
	db, cur = db_set(request)
	sql = "SELECT * FROM wec_members WHERE user = '%s' and password = '%s' and DB = '%s'" %(user1,password1,company1)
	cur.execute(sql)
	tmp = cur.fetchall()
	try:
		type1 = tmp2 = tmp[0][3]
		...
		type1 = ''
		tmp2 = 0
	request.session['active_type'] = type1
	db.close()
	return tmp2

def login2(request):
	try:
		test = request.session['form_check1']
	except:
		request.session['form_check1'] = 0
	if request.session['form_check1'] == 1:
		data1 = request.session['form_data']
		user1 = data1[0]
		company1 = data1[1]
		password1 = data1[2]
		request.session['form_check1'] = 0
		check1 = password_check(request,user1,company1,password1)  # Check Password and return session 'pwd_check1'
		if check1 == 0:
			return render(request,'dashboard_fail.html')
		else:
			request.session["active_user"] = user1
			request.session["active_db"] = company1
			request.session["active_admin"] = "admin"
			return render(request,'dashboard_main.html')

	else:
		form_var= ['User','Company','Password']
		form_type = ['text','text','password']
		form_var3 = list(zip(form_var,form_type))
		request.session['form_var'] = form_var3
		request.session['form_title'] = 'Login'  # Title of the dialog box
		request.session['extends1'] = 'main.html'  # Extend from this page
		request.session['form_color'] = '#000000'  # Color of form dialog box
		request.session['form_font_color'] = '#ffffff' # Font color
		request.session['form_redirect'] = '/login2'  # Redirect back here
		return render(request,'redirect_form_action1.html')

# Template for generic Form taking in information and using form_template.html 
def form_action1(request):
	var1 = []
	request.session['blank'] = None
	if request.POST:
		for i in request.session['form_var']:
			x = request.POST.get(i[0])
			var1.append(x)
		request.session['form_check1'] = 1
		request.session['form_data'] = var1
		return render(request,'redirect_template.html')
	else:
		form = Members_Login_Form()
	args = {}
	args.update(csrf(request))
	args['form'] = form
	return render(request,'form_template.html', args)
